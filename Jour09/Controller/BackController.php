...
class BackController extends AbstractController{
    ...
    public function projet_new(){
        ...
        if(!empty($_POST)){
            ...
            if(strlen($nom) < 2 || strlen($nom)> 255){
                $erreur[] = "le champ nom doit contenir au minimum 2 caractères et au maximum 255 caractères"; 
            }

            if((int)$duree < 1 || (int)$duree > 100 ){
                $erreur[] = "la durée doit être un chiffre entre 2 et 99";
            }

            $unites = ["jour", "semaine", "mois" , "année"];
            if(!in_array($unite, $unites)){
                $erreur[] = "veuillez sélectionner une valeur dans le menu déroulant" ; 
            }

            if(empty($technos)){
                $erreur[] = "vous devez sélectionner au moins une techno pour votre projet"; 
            }
        ...
        if(count($erreur) === 0 && !empty($_POST)){
            ...
            foreach($technos as $techno_id){
                BDD::getInstance()->query("INSERT INTO projets_technos (id_projets , id_technos) VALUES ( :projet_id , :techno_id )", [
                    "projet_id" => $id ,
                    "techno_id" => $techno_id
                ]);
            }
    ...
    public function projet_delete( string $id){
        ...
        BDD::getInstance()->query("DELETE FROM projets_technos WHERE id_projets = :projet_id" , ["projet_id" => $id]) ;
    ...
    public function projet_update(string $id){
        ...
        $sql = "SELECT p.nom, p.duree, p.unite , GROUP_CONCAT( pt.id_technos ) AS technos
        FROM projets AS p 
